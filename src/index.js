 ...
 *
 * A profile installs only its own declared dependencies, so schemastery reaches
 * this package through the host's dependency graph rather than through its own.
 * When that fails there is no configuration page — but the shutdown button, the
 * card, and the switch itself all keep working, so this is reported and not
 * fatal. It is reported because the failure is otherwise invisible: the only
 * symptom is a mode that does not survive a cold start.
 * @param note - the shared log sink.
...
async function buildSchema(note) {
  try {
    ...
    if (typeof z?.object !== 'function') {
      note('schemastery resolved but exposes no object() builder; the settings section is skipped')
      return undefined
    }
    return z.object({
    ...
    note(`schemastery is not resolvable from this install (${error.message}); the settings section is skipped, so a switched mode is not persisted across a restart`)
    return undefined
  }
}
