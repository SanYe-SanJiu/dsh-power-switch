...
export function apply(ctx, config = {}, timings = {}) {
  ...
  /**
   * Take a configuration the host's settings store published.
   *
   * `mirrors` is load-bearing, not cosmetic. The plugin records the launch mode
   * itself for the readers that run OUTSIDE the host, and this sink is one of the
   * two events allowed to write that record; the other is the person's explicit
   * choice. A host that generates no settings form for this plugin publishes the
   * RAW LOADER ROW in its place — nobody chose that value — and mirroring it
   * overwrote the record the card had just written: a mode set to `app` reverted to
   * `tab` at every host start, and survived only the restart that also passed the
   * mode down as an environment variable. A cold start has no such variable, so it
   * read the clobbered file and opened a tab.
   * @param next - the resolved configuration.
   * @param mirrors - whether the value came from a real user layer.
   */
  const publishFromStore = (next, mirrors = true) => {
    current = applyRecorded(next)
    if (mirrors !== true) return
    try {
      writeRecordedLaunchMode(next.launchMode)
    } catch {
      // A record that cannot be written costs persistence, not the boot.
    }
  }
   ...
   * that race.
   *
   * The plugin's own record is in the chain because DSH 0.1.7 retired the
   * settings document: on such a host the document answers nothing, and without
   * the record a stored "app" would silently open a tab on the next launch.
   * @returns `'app'` or `'tab'`.
   */
  const startupMode = () => resolveLaunchMode(settingsPath()) ?? current.launchMode

  // Seed the record when it is missing, so a host without a settings document
  // still has something to read on the next cold start. Never overwritten here:
  // an existing record is the user's last explicit choice, and the document (if
  // the host has one) is mirrored over it above.
  if (readRecordedLaunchMode() === null) {
    try {
      writeRecordedLaunchMode(startupMode())
    } catch {
  ...
  /**
   * Persist one launch mode, in the plugin's own record AND the host's store.
   *
   ...
   *
   * The plugin's own record is written before that, and unconditionally: it is
   * what a cold start reads on a host whose settings store no longer carries
   * plugin configuration (DSH 0.1.7 moved it into the profile patch and retired
   * `settings.yaml`). Without it, "switched to the app window" survives until the
   * process exits and then silently reverts to a tab.
   * @param mode - `'app'` or `'tab'`.
   * @returns whether the host's settings store also accepted it — `false` when no
   *   provider is attached or this DSH version has no writable plugin settings
   *   (the choice still governs the next launch, through the plugin's record).
   */
  const persistLaunchMode = async (mode) => {
    ...
    try {
      ...
      note(`could not record the launch mode for the next cold start (${String(error?.message ?? error)})`)
    }
...
/**
 * Offer the plugin's configuration surface, on either settings model.
 *
 * DSH 0.1.6 and earlier: a registered namespace with a schema, a write scope and
 * an installed section. DSH 0.1.7 replaced that with generated forms — the
 * service loses `register`/`installSection` and exposes `describe`/`update`
 * instead. The two paths are detected, not version-sniffed, because a host can
 * also be built from a checkout at any version.
 *
 * Either way this is a convenience: absent schemastery, absent a settings
 * service, or a host with neither API, the shutdown button, the card and the
 * mode switch keep working — and the choice is recorded by the plugin itself for
 * the next launch.
 * @param settingsCtx - context carrying the settings service.
 * @param owner - the plugin context that owns the section's lifetime.
 * @param config - the loader-row configuration.
 * @param publish - sink receiving each newly resolved configuration; its second
 *   argument says whether that value came from a real user layer.
...
function installSettingsSection(settingsCtx, owner, config, publish, expose, note) {
  void (async () => {
    ...
    if (typeof settings.register !== 'function') {
      installGeneratedForm(settings, config, publish, expose, note)
      return
    }
    ...
    const scope = settings.register('dsh-power-switch', schema, { base: config })
    expose(scope)
    try {
      ...
      settings.installSection(owner, 'dsh-power-switch', schema, config, {
        setSource: (source) => {
...
/**
 * The DSH 0.1.7+ settings path: find this plugin's generated form, write through
 * `update`.
 *
 * There is no registration call any more — a form exists for each Loader entry
 * that declares configuration — so the entry is FOUND rather than announced.
 * Matching on the live value is deliberate: the entry id is chosen by the
 * profile's composition (this package's patch row is `power-switch`, and nothing
 * promises that stays the id), while the shape of the value is this plugin's own.
 * @param settings - the host's `SettingsForms` service.
 * @param config - the loader-row configuration.
 * @param publish - sink receiving each newly resolved configuration; its second
 *   argument is `false` for a value nobody chose (the raw loader row).
 * @param expose - sink receiving the write adapter.
 * @param note - the shared log sink.
 */
function installGeneratedForm(settings, config, publish, expose, note) {
  const rows = typeof settings.describe === 'function' ? settings.describe() : []
  const row = (Array.isArray(rows) ? rows : []).find((entry) => entry !== null && typeof entry === 'object'
    && entry.value !== null && typeof entry.value === 'object'
    && typeof entry.value.launchMode === 'string')
  if (row === undefined || typeof row.ns !== 'string') {
    note('this DSH generates settings forms and has none for this plugin; the switch still works, and the choice is recorded for the next launch')
    // NOT mirrored — and the recorded choice is ADOPTED. On this host the record is the
    // only place the person's choice lives, so the effective configuration has to follow
    // it; otherwise the card reports `tab` (the row default) while the launcher and the
    // supervisor open the app window the record asks for. That disagreement is part of
    // what made the original overwrite hard to see.
    const fallback = resolveConfig(config, undefined)
    const recorded = readRecordedLaunchMode()
    publish(recorded === null ? fallback : { ...fallback, launchMode: recorded }, false)
    return
  }
  publish(resolveConfig(row.value, undefined))
  if (typeof settings.update !== 'function') {
    note('the host settings service is read-only here; a switched mode still governs the next launch')
    return
  }
  // The same shape the legacy scope has, so the restart route does not need to
  // know which DSH version it is running on.
  expose({ update: (patch) => settings.update(row.ns, patch) })
}
