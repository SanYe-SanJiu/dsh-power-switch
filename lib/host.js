 ...
 *
 * `delayMs` is the configured "wait before exit", and it is honoured here or
 * nowhere: this function is the single place the timings are decided, and for two
 * releases it ignored the value while the card happily saved and displayed it --
 * every delay measured 1200/7700 ms, whatever the setting said. The configured
 * value is a HOLD, floored by the grace the answer needs to reach the browser: a
 * delay of 0 must not race the response it belongs to, so 0 means "as soon as the
 * answer is out", which is `flushMs`.
 *
 ...
 * case that used to leave the process alive while the page waited. The watchdog
 * keeps its distance from the graceful request, so it still sits above the CLI's
 * own 5 s disposal bound however long the person asked to wait.
 * @param options - the exit code, the configured hold, and whether to skip graceful disposal.
...
export function createExitPlan(options, flushMs = RESPONSE_FLUSH_MS, watchdogMs = EXIT_WATCHDOG_MS) {
  ...
  // An absent or malformed hold means none: the caller that has a configured value
  // is `readShutdownRequest`, which always fills it in.
  const delayMs = Number.isFinite(options.delayMs) ? Math.max(0, Math.trunc(options.delayMs)) : 0
  const leaveAtMs = Math.max(flushMs, delayMs)
  return {
    hard,
    /** The configured hold, as the exit plan understood it. */
    delayMs,
    /** When the graceful request is made, or null when there is none. */
    gracefulAtMs: hard ? null : leaveAtMs,
    /** When the process is ended unconditionally. */
    forceAtMs: leaveAtMs + (hard ? 0 : watchdogMs),
  }
}
...
export function createExitResponder(deps) {
  ...
  return (response, status, payload, options) => {
    ...
    note?.(`exit requested: code=${String(options.code)} hard=${String(plan.hard)} delay=${String(plan.delayMs)}ms gracefulAt=${String(plan.gracefulAtMs)}ms forceAt=${String(plan.forceAtMs)}ms graceful=${typeof gracefulExit === 'function' ? 'yes' : 'no'}`)
...
/** Route the launcher opens for an app window: it sizes the window, then hands over. */
export const APP_WINDOW_ROUTE = '/api/dsh-power-switch/app-window'

/** The URL fragment that marks a window as one the launcher opened for app mode. */
export const APP_WINDOW_HASH = '#dsh-power-switch-app'

/**
 * The hand-over URL an app window may be opened with, or null when it may not.
 *
 * The launcher opens the window on this plugin's own page rather than straight on
 * DSH, because the browser shows a window the moment it is created and a page that
 * resizes itself as its FIRST action does it before anybody can see the default
 * size (measured: the window was already the filled size at the first sample, 165 ms
 * after launch, where resizing from inside DSH's own boot flashed the half-width
 * window for as long as the app took to render).
 *
 * Only a loopback URL that carries a token may be handed over to, and only one
 * without characters that could close the quoting of the page: this is a redirect a
 * browser follows, so anything else here would be an open redirect.
 * @param raw - the `next` query parameter, or undefined.
 * @returns the absolute URL to redirect to, or null when it is not acceptable.
 */
export function appWindowTarget(raw) {
  if (typeof raw !== 'string' || raw === '') return null
  // Rejected before parsing: these are what a redirect target must never contain,
  // and a real token URL has none of them.
  ...
  if (!LOOPBACK.has(url.hostname) && url.hostname !== 'localhost') return null
  if (!/^[0-9]{1,5}$/u.test(url.port)) return null
  if (!url.searchParams.has('token')) return null
  return url.href
}

/**
 * The page an app window opens on: fill the work area, then hand over to DSH.
 * @param target - an absolute URL from {@link appWindowTarget}.
 * @returns the HTML document.
 */
export function appWindowPage(target) {
  // The URL travels as a JSON string literal so that nothing in it can end the
  // script element; `appWindowTarget` has already refused quotes and angle brackets.
  const handOver = `${target}${APP_WINDOW_HASH}`
  const literal = JSON.stringify(handOver).replace(/</gu, '\\u003c')
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>DSH</title>
<style>
html, body { margin: 0; height: 100%; background: #1b1b1b; color: #8a8a8a;
...
<p>Starting DSH…</p>
<script>
  // FIRST, before anything else: the browser has already shown this window at its
  // default size, and this is the earliest a page can change that.
  try {
    window.moveTo(0, 0);
    window.resizeTo(window.screen.availWidth, window.screen.availHeight);
  } catch (error) {
    // A window that refuses keeps the size the browser chose; DSH's own page tries
    // again once it is up.
  }
...
/**
 * Build the route that hands an app window over to DSH after sizing it.
 *
 * Read-only GET, and fenced like every other read: a window opening this route is a
 * top-level navigation from this machine.
 * @returns a `webServer` route handler.
 */
export function createAppWindowHandler() {
  return (request, response) => {
    ...
    const target = appWindowTarget(new URL(request.url ?? '/', 'http://127.0.0.1').searchParams.get('next') ?? undefined)
    if (target === null) {
      sendJson(response, 400, { ok: false, error: 'next must be a loopback http URL carrying a token' })
      return
    }
    const body = Buffer.from(appWindowPage(target), 'utf8')
