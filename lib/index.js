  ...
  ctx.inject(['settings'], (settingsCtx) => {
    installSettingsSection(
      ...
      (next) => {
        ...
        // settings document at all (DSH 0.1.7).
        try {
          writeRecordedLaunchMode(next.launchMode)
        } catch {
          // The document still decides while it exists; only a host without one
          // loses this, and it reports that when a switch is made.
        }
      },
...
function installGeneratedForm(settings, config, publish, expose, note) {
  ...
  if (row === undefined || typeof row.ns !== 'string') {
    ...
    publish(resolveConfig(config, undefined))
    return
  }
