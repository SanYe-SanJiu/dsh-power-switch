...
function installGeneratedForm(settings, config, publish, expose, note) {
  ...
  if (row === undefined || typeof row.ns !== 'string') {
    ...
    // NOT mirrored: this is the raw loader row, which nobody chose. Mirroring it is
    // what reverted a stored `app` to `tab` at every host start.
    publish(resolveConfig(config, undefined), false)
    return
  }
