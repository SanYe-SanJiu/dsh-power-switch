...
- **不要在"还没能力完成"的时候先许诺。** 重启助手先写握手文件（=告诉宿主"有人接手了，你可以退"）
  再去开子进程。子进程启动失败是**异步**的，于是宿主退出、无人接手、DSH 永久下线。
  现在校验（启动命令是否可用）放在握手**之前**：不行就拒绝重启，宿主继续服务。
- **宿主自己能说清楚的事情，不要靠拼路径。** 早期版本拼 `<检出>/apps/cli/lib/bin.js`，
  只在源码检出里存在。现在由宿主进程记录自己的 argv/execPath/cwd（`boot.json`）并原样重放。
- **"在我机器上能用"不是"在别人机器上能用"。** 发布前逐条查出了三处同类假设，都改成
  运行时自报或全路径搜索：① 端口写死 3080 → 宿主把自己的真实端口传给助手；
  ② 浏览器只找 `%ProgramFiles%` → 用户级安装（`%LOCALAPPDATA%`）优先；
  ③ `.vbs` 写死 `%ProgramFiles%\nodejs\node.exe` → 宿主记录 `process.execPath`（nvm/fnm/volta/Store 都能起）。
  同类还有：宿主输出的日志位置由**启动它的人**决定，所以既要读宿主自己记的 URL，也要按
  `boot.json` 里的 cwd 去扫日志——只扫一个位置就会在别人的机器上报"找不到宿主"。
- **运行时状态不要写进包目录。** 日志里有 `?token=…`，写在包里就等于写进版本库；
  而且包可能位于只读 store。统一放 `$DSH_HOME/storages/dsh-power-switch/`。
- **`os` 字段要说实话。** 代码只有 Windows 能跑全，就别声明三平台——否则别人装上得到的
  是一半功能静默失效。
- **插槽是共享的**。`sidebar.footer.action` 里还住着 `dsh-cost-meter` 的预算框
  （`min-width: 148px`）；往同一行加东西**不要声明宽度**（用 `flex: none; width: auto`），
  否则会把邻居挤没。本插件的按钮固定 36px。
- **`slots.inject` 要包 try/catch**：某个组合没有这个座位时必须能跳过，卡片与路由照常工作。
- **模板字符串里不能出现反引号**。本包有两处大段模板字符串（CSS、生成的监督进程源码），
  注释里写反引号会把字符串提前截断，而 `node --check` 只检查外层文件——两处都踩过。
- **`shell.Run "cmd /c " & cmd` 的引号陷阱**：要写成
  `shell.Run "cmd /c """ & cmd & """", 0, False`，并且 `.vbs` 内部每一段引号都要成对
  （少一个收尾引号同样静默失败）。
- **`.vbs` 只允许 ASCII**：wscript 按 ANSI 解码，中文会变成乱码图标名；中文一律由宿主以参数
  传进去（参数是 UTF-16）。
- **分类逻辑要放在可测试的 JS 里**，别写在 `.vbs` 里——写在 VBS 里就没法单元测试，
  而"没法测试的分类器"正是漏掉真实快捷方式的原因。
- **不要用 PowerShell 的文本管道改本仓库的非 ASCII 文件**：PS 5.1 的 `Get-Content -Raw`
  按 ANSI 解码、`Set-Content` 写回就成乱码。用编辑器。
